Spring Cloud는 **클라우드 네이티브 플리케이션을 만들기 위한 편리한 도구**이다. 그럼 **클라우드 네이티브** 무엇인가? 클라우드에서 이용하여 설계된 시스템이나 서비스하는 것을 말한다. 결국 여러 작은 서비스를 결합하여 큰 시스템을 구축하는데 필요한 유용한 도구이라고 볼 수 있다.
